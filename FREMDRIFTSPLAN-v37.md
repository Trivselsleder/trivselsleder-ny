# Fremdriftsplan v37 — Trivselsleder AS

**12. august 2026 · Bygget på v36 — status og opprydding per 12. august foldet inn; Fase 3-byggeplanen er nå den aktive planen**

## 0. Om dette dokumentet

**Hva v37 er.** v36 med statusen per 12. august foldet inn, og en opprydding Kjartan ba om: den aktive planen skal vise det som faktisk gjenstår, ikke det som er gjort. Endringene:

- **Kursplanleggeren (kap. 8) og sikkerheten (kap. 9) er FERDIGE og bevist live** (10.–12. august): hall-importen med «Vanlig vertskap» og «Alternative haller» + fjerning av adresse/pris (migr. 021), sikkerhets-vaktene på `hent_evalueringer_admin` og `sett_kort_status` + CHECK på `kort_status` + strammet mottaker-policy (migr. 020), og to pilot-forbedringer — purring/påminnelse/evaluering treffer nå skolens *nåværende* hovedkontakt (ikke den frosne fra invitasjonen), og RA kan velge å ta med øvrige TL-ansvarlige på påminnelsen. Alt flyttet til Vedlegg D.
- **Testkontoen `kjartaneide@me.com` er lukket** — passordet er byttet av Kjartan. Den står ikke lenger som åpent sikkerhetspunkt.
- **Kap. 7 er skrevet om:** pilot-tidslinjen er erstattet av **Fase 3-byggeplanen (steg 0–11)**, som nå er den aktive arbeidsrekkefølgen fram til lansering. Full plan i prosjektnotatet `FASE3-BYGGEPLAN.md`.
- **QA-gjennomgangen 12. august** (ingen løse tråder av betydning; to lavprioritets-rester ligger i Vedlegg D) er innarbeidet; migrasjonsgap-*beviset* (gjenoppbygging fra `019`) står igjen som eget steg i byggeplanen.

Ingen omstrukturering ellers — samme fire deler.

**Hva v36 er.** v35 med pilotarbeidet 10. august foldet inn. Kursplanleggerens
pilot-restarbeid ble bygget og bevist live samme dag: sesong-rubrikken (N7),
filterrad + eksport i kurslista (N4), «Ikke ønsket» som fjerde kortstatus (N2),
og den store jobben — lagring og frysing av kortantallet på kursdagen (N1).
Kulturkortet ble samlet til én liste med kilde-filter (bestillinger lagres nå i
databasen, ikke bare som e-post), migrasjonsgapet ble lukket med et fullt
live-skjema (`019`), og en skjult 403-feil i den delte vakten ble rettet — og
skjemadriften i `profiles` (migrasjon 005/006 var aldri kjørt) løst. Alt er
flyttet til **Vedlegg D**. Ingen omstrukturering — samme fire deler som v35.
Den aktive planen (Del 2) er nå en ren liste over det som gjenstår før pilot:
hall-importen (8.5), sikkerhetsrestene (kap. 9), og loop-test + opprydding før
drift (8.8).

**Hva v35 var.** v34 med alle beslutninger tatt til og med 9. august foldet
...
### 5.1 Statusoversikt (per 12. august 2026)
...
| Kursplanlegger (steg 1–5 + agenttest) | FERDIG til pilot — N1/N2/N4/N7 + samlet kulturkortliste, hall-import (vertskap) og to pilot-forbedringer (nåværende hovedkontakt + TLA-valg) bygget og bevist 10.–12. aug (Vedlegg D) |
| Hallregister (161) + Kursholderregister (17) | FERDIG — «Vanlig vertskap»/«Alternative haller» importert, adresse/pris fjernet (migr. 021, Vedlegg D) |
| Sikkerhet (kap. 9) | FERDIG — RLS-vakter på DEFINER-funksjoner + CHECK på `kort_status` (migr. 020), API-nøkler rotert, testkonto lukket; bevist live (Vedlegg D) |
| Evaluering — grunnmodul + Fase 2 (Del 1–4) | Kjørt i RA-gjennomgang — evaluering + Eivind-varsel sendt og bevist live (Vedlegg D); se 5.6 |
...
| Fase 3 — Ressursbibliotek | UNDER BYGGING — aktiv plan er byggeplanen steg 0–11 (kap. 7 og `FASE3-BYGGEPLAN.md`) |
...
### 5.6 Evaluering (utsending BEVIST; admin-delen kodeverifisert)

Lukker sirkelen invitasjon → påmelding → påminnelse → kurs → evaluering.
Selve evaluerings-utsendingen (skjema, tre vurderinger, gullkorn og
Eivind-varsel ved kjøpsinteresse) er bevist live i RA-gjennomgangen (Vedlegg D).
Resten — spørsmålsadministrasjon, pakker, semester, admin-oversikten — er lest i
koden og ennå ikke fullt gjennomtestet.
...
levert» (vedlegg D). Punkt 2 og 4 (media-sprik, Aktiv læring hjem) er nå
formulert som sendeklare spørsmål til Jon (`sporsmaal-til-jon.md`); punkt 1 og 3
er interne avklaringer. Beslutning #2/#4 blokkerer importen (byggeplanen steg 8),
ikke strukturarbeidet.*
...
Den aktive arbeidsrekkefølgen for Fase 3 er byggeplanen **steg 0–11** (full
versjon i prosjektnotatet `FASE3-BYGGEPLAN.md`). Kort:

- **Steg 0 (nå):** rydding + spørsmålslista til Jon sendes, Bunny.net-konto, CLAUDE.md, hverdagsark.
- **Steg 1:** skallet — skolens innloggede faner, tomme først, med Claude Design.
- **Steg 2:** datamodellen («hyllene») med hele vern-sjekklista + ~20 testleker (største enkeltjobb).
- **Steg 3:** biblioteket på skallet med testlekene — filter, søk, PDF fra data, Bunny-video, brukslogg.
- **Steg 4–5:** TL-hjulet og Periodeplanen fra bunn, tomme, peker-modell, på testdata.
- **Steg 6–7:** importskriptene bygges/øves på juni-eksporten + bevis databasens oppskrift (`019`).
- **Steg 8 (månedsskiftet aug/sep):** blokkere avklart med Jon → fersk eksport + innholdsfrys → full import.
- **Steg 9 (parallelt):** forside, Evidence, Claude Design-rekkefølgen; «Månedens lek»/mest kjøpt/mest lekt.
- **Steg 10:** stor import av skoler/ansatte/nettverk, HubSpot-synk aller sist.
- **Steg 11 (mål ca. 1. oktober):** WCAG/GDPR/Feide/DNS → norsk LANSERING.
- **Jan/feb 2027:** første reelle drift-test på ett fylke. Deretter Del 3 — svensk side først.

Kritisk sti: steg 2 (største byggejobb), designsporet, og at steg 8 er låst til
Jons ferske eksport (kan ikke skje før månedsskiftet aug/sep). Steg 2–6 bør i
praksis være ferdige når eksporten lander.
...
**Må til pilot (medio august) — FERDIG.** Alt dette er bygget og bevist (kap. 8
og 9, Vedlegg D): restpunktene i kursplanleggeren (filterrad/eksport, sesong,
kortutdeling, hall-import), sikkerheten (tilgangssjekker, CRON_SECRET,
API-nøkler, RLS-vakter migr. 020), full loop-test (migr. 022), og sletting av
demo-/testspor (verifisert 0). Hverdagsarket (kap. 14.3) er utarbeidet (utkast klart, `hverdagsark-ansatte.md`).
Statushistorikk for skoler + frafallsvarsler aktiveres når de første ekte
skolene importeres (byggeplanen steg 10) — justert fra «fra pilot» i v36, siden
det ikke finnes ekte skoler før importen (ført i Vedlegg A; jf. kap. 12.3).
...
> **Kapittel 8 er FERDIG til pilot (10.–12. august) — beholdt som referanse for
> kursplanleggeren; full byggestatus i Vedlegg D.** Levert og bevist live:
> sesong (N7), filterrad + eksport (N4), «Ikke ønsket» (N2), lagring/frysing av
> kortantall (N1), samlet kulturkortliste (8.6), hall-import med «Vanlig
> vertskap»/«Alternative haller» + fjernet adresse/pris (8.5, migr. 021), og to
> pilot-forbedringer (purring/påminnelse/evaluering treffer nå skolens nåværende
> hovedkontakt, og RA kan ta med øvrige TL-ansvarlige på påminnelsen). Sidefunnet
> om `kort_status`/`antall_kort` som bare fantes i basen er dokumentert i `019`
> (jf. kap. 12.2). Datamodellen (8.1) og skolens opplevelse (8.3) beholdes som
> referanse; øvrig byggestatus ligger i Vedlegg D.
...
**Merk om `antall_kort` (oppdatert 10. august):** feltet er nå i bruk. Det ble
bekreftet å finnes i den kjørende databasen (som `sesong` — begge levde bare i
basen, ikke i migrasjonsfilene), og lagring + frysing på kursdagen er bygget og
bevist live (kapittel 8.6, Vedlegg D). Tallet er levende (regnes som
`ceil(antall_tl × 1,1)`) til kursdagen, så låses det. Migrasjonsfil `017`
dokumenterer kolonnen, og `019` dokumenterer hele live-skjemaet (kapittel 12.2).
...
| Steg 4 — RA-admin | Ferdig — eksport levert 10. aug (8.4, Vedlegg D); flytteforespørsel gjenstår (kapittel 17) |
...
| Hallregister (161 haller) | FERDIG — «Vanlig vertskap»/«Alternative haller» importert (migr. 021, Vedlegg D) |
...
| Kortutdeling | FERDIG 10. aug — lagring/frysing av kortantall, «Ikke ønsket»-status, samlet kulturkortliste med kilde-filter og kryssjekk mot forhåndsbestillinger bygget og bevist live (8.6, Vedlegg D) |
...
| # | Tiltak | Når |
|---|---|---|
| 1 | **Filterrad over lista** — nedtrekk for RA, sesong og nettverk, pluss søkefelt på kursnavn og hall | Ferdig 10. aug (Vedlegg D) |
| 2 | **Eksport til regneark** fra kursoversikten | Ferdig 10. aug — CSV av de filtrerte radene, norsk Excel (Vedlegg D) |
...
**Punkt 1 og 2 levert 10. august (Vedlegg D).** Filterraden søker på kursnavn
og hallnavn, nedtrekk for RA, nettverk og sesong, teller «Viser X av Y kurs»,
og CSV-eksporten skriver de filtrerte radene (BOM + semikolon + UTF-8 for norsk
Excel). Sesong-forutsetningen som lå under punkt 1 er løst i samme slengen:
feltet `kurs.sesong` (som fantes i basen, men uten inntastingsrubrikk,
`AdminKursplanlegger.jsx:31`) fikk et nedtrekk «Vår/Høst + år» i skjemaet og et
matchende «Alle sesonger»-filter i lista.
...
> **Status 12. august: FERDIG (migr. 021, Vedlegg D).** «Vanlig vertskap» og
> «Alternative haller» er importert, vises i lista og kursskjemaet, og foreslås
> som hint når RA velger hall. Spøkelsesfeltene `adresse` og `pris` er fjernet
> fra `AdminHaller.jsx`. Importjobbene beskrevet under er dermed utført —
> beholdt som referanse.
...
**Bygget og bevist 10. august (prototypen manglet fire ting — nå levert;
full logg i Vedlegg D):**

1. **Lagring og frysing av tallet.** `antall_kort` lagres nå på kurs_skole-raden
   og fryses automatisk ved midnatt på kursdagen (nytt endepunkt
   `api/kurs/frys-kortantall.js` + daglig Vercel-cron; idempotent). Tallet er
   levende (`ceil(antall_tl × 1,1)`) til kursdagen. Manuell overstyring +
   tilbakestilling per rad er også med. Bevist live (commit `b4c39f7`).
2. **«Ikke ønsket» lagt til som fjerde kortstatus.** Statusene er nå fire: Ikke
   behandlet, Fakturer, Gratis, Ikke ønsket (commit `b5123bc`, testet live).
   *Gjenstår å bekrefte i samme spor* (fulgte med dette punktet i v35):
   håndteringen av skoler som ikke har oppgitt antall — ikke eksplisitt
   verifisert, tas med i loop-testen før pilot.
3. **Én samlet liste i stedet for to faner.** Løsningen ble bedre enn to faner:
   ÉN side `/admin/kortoversikt` («Kulturkort — kort og bestillinger») som
   viser både kort fra kurs og bestillinger i posten, med et **kilde-filter**
   (Kurs / Bestilling) — Camillas eget ønske om ÉN liste. De to gamle
   menypunktene er erstattet av dette ene (gamle ruter beholdt). Forutsetning
   som måtte løses først: post-bestillingene ble faktisk ikke lagret noe sted
   (kun e-post + død localStorage) — dårligere enn dagens trivselsleder.no.
   Trinn 1 la dem i en ny DB-tabell `kulturkort_bestillinger` (commits
   `9422111`/`ff72efb`/`c917e75`); Trinn 2 bygget den samlede lista (commit
   `1f5148f`).
4. **Kryssjekken mot forhåndsbestillinger** løses av den samme samlede lista:
   Camilla ser nå kurs-kort og post-bestillinger side om side med kilde, så en
   skole som allerede har bestilt i posten ikke får kort to ganger. Systemet
   viser begge deler på ett sted.
...
150 linjer) var utgangspunktet; den samlede lista bygger videre på den.
Beløp beregnes per kilde (kurs uten porto, bestilling inkludert porto), og
prisinnstillingene er flyttet til den samme siden.
...
- [x] Filterrad + eksport + sesong-rubrikk (8.4, punkt 1–2) — 10. aug
- [x] Kortutdeling: lagring, frysing, statuser, samlet liste (8.6) — 10. aug
- [x] Import av «Vanlig vertskap» og «Alternative haller» + rydd adresse/pris (8.5) — migr. 021, 11. aug
- [x] Sikkerhetspunktene i kapittel 9 — RLS-vakter + CHECK (migr. 020), bevist live (Vedlegg D)
- [x] Full loop-test mot fasit — kjørt, rettelser i migr. 022 (Vedlegg D)
- [x] Demo-video — DROPPET; erstattet av illustrert bruksanvisning/forklaring for RA (Vedlegg D)
- [x] Demo-innhold og testspor slettes — verifisert 0 (11.–12. aug, Vedlegg D)
- [ ] `eivind_epost` satt, `motor_aktiv` → ja, «Utstyrspakker» → Klubben-lenkene — **verdiflipp ved lansering** (kap. 14, ca. 1. oktober)
...
**Restpunktene er nå FERDIGE (11.–12. august, migr. 020 — se Vedlegg D):**

- **`hent_evalueringer_admin` og `sett_kort_status`** har fått caller-vakt
  (rolle-/eiersjekk). Bevist live: en innlogget ansatt får sine rader, `anon`
  får 0.
- **CHECK-constraint på `kort_status`** — kun de fire gyldige verdiene.
- **Mottaker-policyen på `kurs_skole_mottaker`** strammet til ansatt/superadmin.
  Den offentlige svar-flyten går via en SECURITY DEFINER-funksjon med token,
  ikke åpen tabelltilgang — GDPR-punktet dermed lukket.
- **Admin-endepunktene** sjekker nå innlogging FØR kroppen valideres.
- **De fire API-nøklene** i «Min nettside» er rotert av Kjartan (gamle nøkler er dermed døde uansett Git-historikk).
- **Testkontoen `kjartaneide@me.com`** (skoleadmin): passordet er byttet av
  Kjartan. Det hardkodede passordet i `scripts/seed-testbruker.sql` er dermed
  dødt og fjernes ved anledning (kosmetisk).

Full logg i Vedlegg D. De konkrete RLS-restene fra dette kapitlet er dermed
lukket. En full RLS-/GDPR-gjennomgang av de NYE Fase 3-tabellene hører fortsatt
til lanseringen (kap. 14). Den øvrige sikkerhets-forutsetningen er å *bevise*
databasens oppskrift ved gjenoppbygging fra `019` (byggeplanen steg 7).
...
Frem til 10. august fantes bare seksten filer i `supabase/migrations`,
nummerert 001–016; de stoppet i juni — ingen av kursplanleggerens tabeller var
med, og bare 2 av 8 databasefunksjoner. (Se fremgangsnoten under: 017–019 er nå
lagt til.) *På
...
> **Fremgang 10. august (se Vedlegg D).** Gapet er kartlagt og i hovedsak
> lukket: (a) filene som var skrevet men aldri kjørt (migrasjon 005 + 006 —
> `profiles.epost`/`aktiv` + rolle-constraint) ble kjørt mot live-basen, og
> (b) hele det kjørende skjemaet ble eksportert til `019_live_schema.sql`
> (19 tabeller, 29 funksjoner, 37 policyer, 53 constraints) via en midlertidig
> introspeksjons-funksjon — uten Docker/pg_dump/DB-passord. Oppskriften finnes
> nå. **Det som gjenstår** er selve beviset: bygge en kopi fra `019` og
> sammenligne (`019` er KODEVERIFISERT — dumpet fra basen, ikke ennå prøvd
> gjenoppbygd). Gjøres før den store dataimporten.
...
### v36 → v37 (12. august 2026)

v37 = v36 med statusen per 12. august foldet inn. Ingen omstrukturering — samme
fire deler. Fullførte punkter flyttet til Vedlegg D (regel 5).

| Endring i v37 | Hvor |
|---|---|
| Kap. 8 (kursplanlegger) markert FERDIG til pilot; hall-import (vertskap/alt. haller, migr. 021) + to pilot-forbedringer (nåværende hovedkontakt + TLA-valg) levert; datamodellen beholdt som referanse | Kap. 8, Vedlegg D |
| Kap. 9 (sikkerhet) markert FERDIG: caller-vakter på `hent_evalueringer_admin` + `sett_kort_status`, CHECK på `kort_status`, strammet mottaker-policy (migr. 020), API-nøkler rotert | Kap. 9, Vedlegg D |
| Testkontoen `kjartaneide@me.com` lukket (passord byttet) — ute av åpne sikkerhetspunkter | Kap. 9 |
| Kap. 7-tidslinjen erstattet av Fase 3-byggeplanen (steg 0–11); full plan i `FASE3-BYGGEPLAN.md` | Kap. 7 |
| QA 12. aug innarbeidet (ingen løse tråder); migrasjonsgap-*beviset* gjenstår som steg 7 i byggeplanen | Kap. 7, 12.2 |
| Sjekklista 8.8 og 5.1-statustabellen oppdatert til 12. aug | Kap. 8.8, 5.1 |
| Loop-test kjørt (rettelser migr. 022); testinnhold slettet og verifisert 0; demo-video DROPPET → erstattet av illustrert bruksanvisning/forklaring for RA | Kap. 8.8, Vedlegg D |
| Statushistorikk/frafallsvarsler flyttet fra «fra pilot» til «fra første ekte skole» (byggeplanen steg 10) — ingen ekte skoler før importen | Kap. 7.2, 12.3 |
| Verdiflippene (`eivind_epost`, `motor_aktiv`→ja, Klubben-lenkene) presisert som lanserings-verdiflipp (kap. 14), ikke pilot | Kap. 8.8, 5.10 |
| Evaluering oppgradert: kjørt i RA-gjennomgang (evaluering + Eivind-varsel bevist live) | Kap. 5.1, 5.6 |

Detaljer i prosjektnotatene: `PRE-FASE3-QA-12aug.md`, `FASE3-BYGGEPLAN.md`,
`STATUS-11aug-opprydding-ferdig.md`.

### v35 → v36 (10. august 2026, kveld)

v36 = v35 med pilotarbeidet 10. august foldet inn. Ingen omstrukturering —
samme fire deler. Fullførte punkter er merket og flyttet til Vedlegg D (regel
5), så Del 2 igjen bare viser det som gjenstår.

| Endring i v36 | Hvor |
|---|---|
| Kursplanleggerens pilot-restarbeid bygget og bevist live: sesong (N7), filterrad + eksport (N4), «Ikke ønsket»-status (N2), lagring/frysing av kortantall (N1) | Kap. 8.2/8.4/8.6, Vedlegg D |
| Kulturkortet samlet til ÉN liste med kilde-filter; post-bestillinger lagres nå i DB (var kun e-post/localStorage før) | Kap. 8.6, Vedlegg D |
| `antall_kort`-merknaden i 8.1 og «bygges nå» i 8.2/8.4/8.6 skrevet om til gjeldende sannhet (levert), med pekere til Vedlegg D | Kap. 8 |
| Hall (8.5) merket «satt på vent» — eneste gjenstående pilot-punkt i kap. 8 utover sjekklista; adresse/pris-fjerning besluttet men ikke utført | Kap. 8.5 |
| Sikkerhet: `_vakt.js` 403-feil + `profiles`-skjemadrift (005/006 aldri kjørt) funnet og rettet; `sett_kort_status` (SECURITY DEFINER uten sjekk) + manglende CHECK på `kort_status` lagt til RLS-restlista | Kap. 9, Vedlegg D |
| Migrasjonsgapet (12.2): 005/006 kjørt + `019_live_schema.sql` eksportert — oppskriften finnes nå; gjenstår å bevise ved gjenoppbygging | Kap. 12.2, Vedlegg D |
| Sjekklista før pilot (8.8): to punkter huket av; kap. 5.1-statustabellen datert 10. aug | Kap. 8.8, 5.1 |

Detaljer i prosjektnotatene: `STATUS-kursplanlegger.md`,
`PILOT-kartlagt-mot-kode.md`.
...
### 12. august 2026 — QA før Fase 3 + pilot-forbedringer + testkonto lukket [FERDIG]

- **Pilot-forbedringer (commits `c68b95a`, `2dbd3f9`):** purring, påminnelse og
  evaluering treffer nå skolens *nåværende* hovedkontakt (henter `skoler.hktl_*`,
  faller tilbake til den frosne adressen om nødvendig — mister aldri mottaker),
  og RA kan velge å ta med øvrige TL-ansvarlige på påminnelsen. Bevist live med
  tørrkjøring (bytt hovedkontakt → e-posten følger med; TLA-hake gir 1 vs 2
  mottakere). Speilet i `hvem-star-for-tur`-lista.
- **QA-gjennomgang før Fase 3 (`PRE-FASE3-QA-12aug.md`):** ingen løse tråder i
  kode/base, kontrollert mot git-logg, migrasjonsfiler og live auth. Testkontoen
  `kjartaneide@me.com` (skoleadmin) — **passordet byttet av Kjartan**, den ene
  reelle tråden lukket. Lavprioritets-rester: bevis migr. `019` ved
  gjenoppbygging (byggeplanen steg 7), slett frakoblet pakkebilde i storage.
- **Ny plan:** `FASE3-BYGGEPLAN.md` (steg 0–11) redesignet og uavhengig
  kontrollert; blir kap. 7 i v37.

### 11.–12. august 2026 — Sikkerhet kap. 9: RLS-vaktene lukket [FERDIG]

- **Migrasjon `020`:** `hent_evalueringer_admin` og `sett_kort_status` fikk
  caller-vakt (kun ansatt/superadmin); CHECK-constraint på `kort_status` (fire
  gyldige verdier); `kurs_skole_mottaker`-policyen strammet til ansatt/superadmin;
  admin-endepunktene sjekker innlogging før kroppen. Bevist live: `anon` får 0
  rader på evalueringene.
- **`inviter-bruker`** avviser nå deaktivert konto (commit `297d706`).
- **API-nøklene** i «Min nettside» rotert av Kjartan (SerpAPI + Anthropic).

### 11. august 2026 — Hall (kap. 8.5) + loop-test-rettelser [FERDIG]

- **Migrasjon `021`:** «Vanlig vertskap» og «Alternative haller» importert; vises
  i hall-lista og kursskjemaet; foreslås som hint når RA velger hall (commits
  `8beaa57`/`a5170d0`). Spøkelsesfeltene `adresse`/`pris` fjernet.
- **Migrasjon `022` (loop-test-rettelser):** FK-kaskader ved sletting av kurs,
  `flytt_skole_til_kurs` nekter dublett, `kopier_kurs` tar med oppmøtetider,
  sesong-kolonne i lista, deaktiverte kursholdere i nedtrekk, utvidet slett-advarsel.
- **Testinnhold slettet og verifisert 0** (5 test-kurs, 21 test-skoler, 13
  evalueringer, 21 auth-testkontoer); hallregisteret og de ekte skolene urørt.
- **Demo-video droppet** — erstattet av illustrert bruksanvisning/forklaring for RA.

### 10. august 2026 (kveld) — Kulturkort samlet: ekte lagring + én liste [FERDIG]

Camillas hovedkrav løst, og en regresjon mot dagens side rettet på veien.

- **Trinn 1 — post-bestillinger lagres nå i databasen** (commits `9422111`,
  `ff72efb`, `c917e75`). Ny tabell `kulturkort_bestillinger` (+ RLS + GRANT).
  `send-bestilling.js` skriver til DB i tillegg til e-post; `AdminBestillinger`
  leser fra DB; død localStorage fjernet. Før dette ble bestillinger kun sendt
  som e-post — dårligere enn dagens trivselsleder.no. **Lærdom:** en ny tabell
  trenger GRANT (authenticated + service_role), ikke bare RLS — ellers
  «permission denied».
- **Trinn 2 — én samlet liste** (commit `1f5148f`): `/admin/kortoversikt`
  («Kulturkort — kort og bestillinger») viser kurs-kort og post-bestillinger
  med **kilde-filter** (Kurs / Bestilling), full redigering for begge, og beløp
  per kilde (kurs uten porto, bestilling inkludert porto). De to gamle
  menypunktene erstattet av ett; gamle ruter beholdt. Prisinnstillingene flyttet
  hit. Kontroll-agent fant 2 feil, begge rettet. Bevist live (begge kilder,
  filter, statusendring).

### 10. august 2026 (kveld) — Migrasjonsgapet lukket [FERDIG]

- Systematisk sammenligning av migrasjonsfiler mot den kjørende basen. Retning A
  (skrevet, men aldri kjørt): migrasjon **005** (`profiles.epost`/`aktiv`) og
  **006** (rolle-constraint skoleadmin/skoleansatt/feide) ble kjørt via
  SQL-editoren. Retning B (kun i basen): 5 tabeller + 22 RPC-er.
- **`019_live_schema.sql`** (commit `87359c6`): hele live-skjemaet eksportert —
  ...
  basen (slettet etterpå). Et testmiljø kan nå bygges fra bunnen (jf. 12.2 —
  selve gjenoppbyggings-beviset gjenstår).

### 10. august 2026 — Sikkerhet: vakt-feil og skjemadrift rettet [FERDIG]

- **`_vakt.js` avviste ALLE innloggede ansatte med 403** (commit `17997a4`):
  den delte vakten fra 5. august slo opp `profiles.aktiv`, en kolonne som ikke
  fantes → spørringen feilet → profil ble `null` → 403 på hvert `_vakt`-
  endepunkt. Skjult siden 5. aug fordi cron-jobbene sjekker CRON_SECRET FØR
  rollen, og 5.-aug-testen bare bekreftet 401 for uinnlogget.
- **`profiles`-skjemadriften løst:** migrasjon 005/006 var aldri kjørt mot
  live-basen → all onboarding (inviter/godkjenn/opprett/Feide) feilet på
  manglende `epost`/`aktiv`. Kjørt via SQL-editoren.
- **Aktiv-sperren gjeninnført** (commit `9522688`): nå som `aktiv` finnes, velger
  vakten `rolle, aktiv` igjen og gir 403 til en deaktivert ansatt. Server-side
  «deaktiver ansatt» virker igjen. Bevist: innloggede ansatte får 200.

### 10. august 2026 — Kursplanleggeren til pilot: N1/N2/N4/N7 [FERDIG]

- **N1 — lagre + fryse kortantall** (commit `b4c39f7`, bevist live). Nytt
  endepunkt `api/kurs/frys-kortantall.js` + daglig Vercel-cron (`0 5 * * *`).
  Tallet (TL + 10 %) er levende til kursdagen, så låses `antall_kort`
  automatisk. Idempotent. Manuell overstyring + tilbakestilling per rad.
  Frontend viser frosset / «beregnes» levende; totaler + faktureringssum
  bruker effektivt tall. Migrasjonsfil `017` dokumenterer kolonnen. Kontroll-
  agent fant + rettet én ekte feil (optimistisk UI ble ikke rullet tilbake ved
  lagringsfeil → skjerm/faktureringssum kunne vise et tall basen ikke lagret).
  Bevist: tørrkjøring (2 aktuelle), ekte frysing (15→17, 8→9), idempotens (0),
  overstyring 12, reset null, -3 avvist.
- **N2 — «Ikke ønsket» som fjerde kortstatus** (commit `b5123bc`, testet live).
- **N4 — filterrad i kursoversikten** (commit `b8f1d5e`, bevist live via
  Claude-in-Chrome): søk på kursnavn/hallnavn, nedtrekk for RA og nettverk,
  teller «Viser X av Y», CSV-eksport av de filtrerte radene (BOM + semikolon +
  UTF-8 for norsk Excel). Klient-side, ingen databaseendring.
- **N7 — sesong-felt** (commit `92ef748`, bevist live): nedtrekk «Vår/Høst + år»
  i skjemaet + «Alle sesonger»-filter i lista. Kolonnen `sesong` bekreftet å
  finnes i basen (REST 200 for `select=sesong`, 400 for en tulle-kolonne) —
  ingen databaseendring. Ende-til-ende bevist: «Høst 2026» satt → skrevet til
  DB → plukket opp av filteret.

### 9. august 2026 — Sikkerhetshullet lukket [FERDIG]
...
### 9. august 2026 — «Inaktiv»-status fjernet [FERDIG]
...
### 9.–10. august 2026 — fire av fem tall talt fra ekte data [FERDIG]
...
### 9. august 2026 — to småting avklart uten kodejobb [FERDIG]
