...
// Viser snitt PER RAD («X,X av 6») på tvers av alle skoler som har svart i runden.
// Data: RPC-en skoleus_resultat_effekt(p_runde) (migr 086) — aggregerer mot rundens
// undersokelse_id (ikke globalt). Gjenbrukbart: skoleus-admin nå, ledelsessiden senere.
  ...
  return (
    <div className={'bg-white border border-gray-200 rounded-xl p-5 ' + className}>
      <div className="flex items-baseline justify-between gap-3 mb-1">
        ...
        <span className="text-xs text-gray-500">Snitt av {SKALA_MAX}</span>
      </div>
      ...
      {laster ? (
        ...
        <ul className="space-y-3">
          {rader.map(rad => {
            ...
            const pct = har ? Math.max(0, Math.min(100, (Number(rad.snitt) / SKALA_MAX) * 100)) : 0
            return (
              <li key={rad.matriserad_id}>
                <div className="flex items-baseline justify-between gap-3 mb-1">
                  ...
                  <span className="text-sm font-semibold text-gray-900 tabular-nums whitespace-nowrap">
                    {formaterSnitt(rad.snitt)} <span className="font-normal text-gray-400">av {SKALA_MAX}</span>
                  </span>
                </div>
                <div
                  className="h-2 rounded-full bg-petrol/10 overflow-hidden"
                  role="img"
                  aria-label={har ? `${formaterSnitt(rad.snitt)} av ${SKALA_MAX}` : 'Ingen svar'}
                >
