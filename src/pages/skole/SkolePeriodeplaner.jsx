...
import { hentPlaner, opprettPlan, kopierPlan, arkiverPlan } from '../../lib/periodeplan'
...
export default function SkolePeriodeplaner() {
  ...
  async function lagPlan() {
    ...
    setLagrer(true)
    try {
      const id = await opprettPlan({ navn: navn.trim(), aar: aar ? Number(aar) : null })
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex items-center justify-between gap-4">
        ...
        {!nytt && (
          <button onClick={() => setNytt(true)} className="shrink-0 bg-magenta text-white font-medium px-5 py-2.5 rounded-full hover:bg-magenta/90 transition">
            + Ny plan
          </button>
        )}
      </div>
        ...
        {sortert.map((p) => (
          <div key={p.id} className="bg-white rounded-2xl border border-gray-200 hover:shadow-md transition p-5">
            <Link to={`/min-side/periodeplaner/${p.id}`} className="block">
              <h3 className="font-bold text-gray-900 hover:text-magenta">{p.navn}</h3>
