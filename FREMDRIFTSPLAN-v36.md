# Fremdriftsplan v36 — Trivselsleder AS

**10. august 2026 (kveld) · Bygget på v35 — pilotarbeidet 10. august foldet inn**
...
### 5.1 Statusoversikt (per 10. august 2026)
...
| Kursplanlegger (steg 1–5 + agenttest) | PILOT-KLAR — pilot-restarbeidet i kapittel 8 i hovedsak fullført 10. aug (N1/N2/N4/N7 + samlet kulturkortliste, se Vedlegg D); hall gjenstår (8.5) |
| Hallregister (161) + Kursholderregister (17) | Ferdig — to importjobber + felt-rydding gjenstår (kapittel 8.5) |
...
> **Pilot-restarbeidet bygget og bevist 10. august (kveld) — se Vedlegg D.**
> Det som 10. august morgen sto som «den ekte jobben» er nå levert og bevist
> live: sesong-rubrikken (N7), filterrad + eksport i kurslista (N4), «Ikke
> ønsket» som fjerde kortstatus (N2), og den store jobben — lagring og frysing
> av kortantallet på kursdagen (N1). Kulturkortet ble samlet til én liste med
> kilde-filter (8.6). **Det som gjenstår i dette kapitlet:** hall-importen og
> felt-ryddingen (8.5), og sjekklista før pilot (8.8). Full byggeliste i
> prosjektnotatet `PILOT-kartlagt-mot-kode.md`; hva som ble levert står i
> Vedlegg D. Et viktig sidefunn er lukket: kortstatus- og `antall_kort`-
> objektene fantes bare i den kjørende databasen, ikke i migrasjonsfilene —
> nå dokumentert i `019` (jf. kapittel 12.2).
...
> **Status 10. august: satt på vent («hall er ferdig for nå», Kjartan).**
> Dette er det eneste gjenstående pilot-punktet i kapittel 8 utover sjekklista.
> Ingen kode er endret her ennå: adresse/pris-feltene står fortsatt i
> `AdminHaller.jsx` (beslutningen om å fjerne dem er tatt, men ikke utført), og
> «Vanlig vertskap»/«Alternative haller» er ikke importert. Begge jobbene står
> derfor åpne under.
...
- [ ] Import av «Vanlig vertskap» og «Alternative haller» + rydd adresse/pris (8.5)
- [ ] Sikkerhetspunktene i kapittel 9 (selve sikkerhetshullet er lukket — se Vedlegg D; RLS-restene gjenstår)
...
- `hent_evalueringer_admin` er SECURITY DEFINER uten sjekk av hvem som spør.
  *På vanlig norsk: den henter alle evalueringssvar og spør aldri hvem som
  ...
  navnet. Alvorlig.*
- **NY 10. august:** `sett_kort_status` er også SECURITY DEFINER uten
  caller-sjekk (gjør bare `UPDATE kurs_skole SET kort_status`). Lavere risiko
  enn evalueringene (å sette en kortstatus er ikke sensitivt), men bør få en
  rolle-/eiersjekk i samme RLS-runde. Merk også: `kort_status` har INGEN
  CHECK-constraint — verdiene styres kun av frontend-lista `STATUSVALG`.
...
> **Løst 10. august (se Vedlegg D):** to sikkerhetsnære feil ble funnet og
> rettet under pilotarbeidet — den delte vakten `_vakt.js` avviste ALLE
> innloggede ansatte med 403 (slo opp en `profiles.aktiv`-kolonne som ikke
> fantes), og skjemadriften bak det (migrasjon 005/006 var skrevet, men aldri
> kjørt mot live-basen) blokkerte all onboarding. Begge er rettet og bevist;
> server-side «deaktiver ansatt» virker nå igjen.
