-- 053: B7 (høring) – levende svarlenke: returner også apen_for_annet_kurs + onske_tekst
-- så skolen kan åpne lenken på nytt og justere UTEN å miste disse feltene.
-- KJØRT LIVE i Supabase 19. aug 2026 (arkiv; basen er allerede endret).
--
-- Husregel 6: retur-signatur endres → drop + recreate + grant. Input (token) er
-- uendret, og JS-klienten leser kolonner på NAVN, så den utplasserte frontenden
-- virker uendret (ignorerer bare de to nye kolonnene).
...
returns table(
  ...
  svart boolean, svart_dato timestamp with time zone,
