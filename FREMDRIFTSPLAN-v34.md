# Fremdriftsplan v34 — Trivselsleder AS

**7. august 2026 · Omstrukturert utgave, bygget på v33 av 6. august**

## 0. Om dette dokumentet

**Hva v34 er.** Samme innhold som v33 — ny struktur. v33 samlet alt som noen
...
4. **Ingenting er fjernet uten beslutning.** Alt innhold fra v33 står enten i
...
| Del 4 (vedlegg A–C) | Endringslogg, historikk, kunnskapsgrunnlag | Oppslagsverk — ingenting her krever handling |

Formen — farger, typografi, topptekst — følger v31-malen, som alltid.
Eldre endringslogger (v24 til v33) ligger i v33 og gjentas ikke her.
...
|---|---|---|---|
| 1 | Fem tall planen er uenig med seg selv om (868/869 leker m.fl.) | Kapittel 10 | Én telling må gjøres før tall brukes utad |
...
| 5 | Skal «Flytt til annet kurs» kun vises for «Kommer ikke»-skoler? | Kapittel 8 | Avklares med RA før pilot |
| 6 | Påminnelse: tidsregel, eller stryke nøkkelen `paaminnelse_dager_for`? | Kapittel 8 | Rydder planen mot koden (anbefaling: stryk) |
| 7 | Statusnavnet «Inaktiv» brukes i koden, men står ikke i statuslista | Kapittel 12 | Må avklares før statushistorikken bygges |
| 8 | Drift etter Kjartan — hvem tar teknisk ansvar? | Kapittel 14 | Del av overleveringen ved lansering |
| 9 | DPA-avkrysning: Supabase, Vercel, Resend + Bunny.net | Kapittel 14 | Lovkrav før lansering |
| 10 | Svensk side: avvik i Lag B | Kapittel 15 | Etter lansering |
| 11 | Kursholder-innlogging: reelt behov? | Kapittel 17 | Etter lansering |
| 12 | Webinar: åpen eller lukket påmelding | Kapittel 20 | Etter lansering |
| 13 | Trivselsundersøkelsen: behandlingsansvar (jurist) | Kapittel 21 | Før undersøkelsen tas i bruk |
| 14 | Abonnementsmodell: per skole / lærer / kombinasjon | Kapittel 22 | Før digital tjeneste selges |
| 15 | Danmark-rapporten: ferdig, men ikke analysert | Kapittel 22 | Etter lansering |

*Nylig avgjorte beslutninger står i endringsloggen — vedlegg A.*
...
**Åpne punkter (fra kapittel 6):** skal «Flytt til annet kurs» kun vises for
«Kommer ikke»-skoler (`SvarOversikt.jsx:293`)? Og: skal påminnelsen ha en
tidsregel, eller strykes nøkkelen `paaminnelse_dager_for` fra planen
(anbefaling: stryk — RA velger dagen og trykker)?
...
To ting gjenstår, begge fra kildefila `Hallregister_utkast_2.xlsx`:

1. **Feltene adresse og pris FINNES** i redigeringsskjemaet
   (`AdminHaller.jsx:311` og `:313`) — men står tomme, fordi kildefila aldri
   hadde dem, og de vises ikke i hall-tabellen (`:152-156` viser bare Navn,
   Kommune, Nettverk). Pris er selve verdien i registeret — det er derfor
   feltene finnes. Dataene må skaffes og kolonnene vises.
2. **To kolonner som fantes i kildefila ble ikke importert:** «Vanlig
   vertskap» (utfylt på 141 av 161 rader) og «Alternative haller» (65 rader).
   Den første er særlig ergerlig: vertskap hukes i dag av manuelt, kurs for
   kurs — mens svaret lå i regnearket hele tiden. Importeres før pilot.
...
## 9. Sikkerhet som må lukkes

*Kun åpne punkter. Alt som er lukket og bevist — tilgangssjekkene 4.–5.
august, hvordan hullene ble funnet — står i vedlegg B.*

### 9.1 HASTER — venter på Kjartan

Sikkerhetsrettingen av `api/kurs`-endepunktene er gjort i koden, men **ikke
pushet**. Til dette hullet er lukket kan hvem som helst på internett hente
skolenavn, kursnavn, datoer, mottakernavn og e-postadresser. Kjartans liste
(kommandoene står i `SIKKERHET-5-august.md`):

1. Sett `CRON_SECRET` i Vercel (Production + Preview).
2. `npm run build`, deretter commit av de 13 filene med den eksplisitte
   fillista — aldri `git add -A`.
3. Sjekk at endepunktet svarer «Ikke autentisert» etterpå.
4. Trekk tilbake GitHub-tokenet i `.git/config` og sett remote-URL på nytt.

### 9.2 Roter API-nøklene

Fire skript i mappa «Min nettside» har API-nøkler hardkodet i klartekst:
`kulturkort_agent_v1.py`, `kulturkort_potensiell_agent.py`,
`skolesjef_agent_v5.py`, `skolesjef_sverige_repair.py`. Samme SerpAPI- og
Anthropic-nøkkel går igjen. **Roter begge nøklene** og sjekk om filene har
vært committet til Git. Gjøres før pilot.

### 9.3 Før pilot

- `hent_evalueringer_admin` er SECURITY DEFINER uten sjekk av hvem som spør.
  *På vanlig norsk: denne funksjonen henter alle evalueringssvar og spør
  aldri hvem som ber om dem. Den er betjenten i luka som utleverer hele
  arkivet til hvem som helst som kjenner navnet på skranken. Alvorlig.*
- `scripts/seed-testbruker.sql` har hardkodet passord, trolig i
  Git-historikken. Brukeren er i aktiv bruk — bytt passord. *På vanlig
  norsk: et passord ble skrevet rett inn i en fil, og filen ligger i
  prosjektets versjonshistorikk. Å slette linjen i dag hjelper ikke —
  historikken husker den. Passordet må byttes.*

### 9.4 Før lansering — RLS-gjennomgangen

Det eldste åpne sikkerhetspunktet i prosjektet. Den må være ferdig FØR den store dataimporten: det er
...
- `anon` har lese- og skriverett på `kurs_skole_mottaker`. *På vanlig norsk:
  ...
  nødvendig for at læreren skal kunne svare uten å logge inn, men det må
  strammes inn slik at en gyldig lenke bare gir tilgang til sin egen rad.
  Personopplysninger — dette er et GDPR-punkt like mye som et teknisk.*
- De fire andre admin-endepunktene validerer kroppen FØR de sjekker
  innlogging. Ingen slipper forbi, men rekkefølgen bør snus. *På vanlig
  norsk: dørvakten sjekker at skjemaet er riktig utfylt før han sjekker at du
  har billett. Ingen kommer inn uten billett — men en uinnlogget person får
  vite om skjemaet var riktig, og det er informasjon han ikke skal ha. Enkel
  retting.*
- Supabase Pro daglige backups er på plass; endringslogg og slettevern bygges i kapittel 10.
...
**ÅPEN BESLUTNING (kapittel 6, nr. 1):** planen er uenig med seg selv om fem
tall — 868 mot 869 leker, 628 mot 537 dokumenter, 1 790 mot 1 792 TL-hjul,
10 428 mot 10 426 periodeplaner, og 1 685 mot 714 kulturkortpartnere.
Sannsynlig forklaring: uttrekk tatt på ulike tidspunkt. Én telling gjøres ved
importen, og så gjelder ett tall gjennom hele planen.
...
Sverige. Det støtter Lag A-konvergensen (kapittel 22.5) — og mediebiblioteket i Supabase (seksjon
...
1. Sortere/forstå dataene — mye er gjort (vedlegg C og kapittel 10.3 + 47 + 48). Gjenstår: de to media-sprikene
...
Besluttet: TL-hjul (1 790) og periodeplaner (10 428) migreres IKKE. Verktøyene bygges
...
Dagens periodeplaner (10 428) og TL-hjul (1 790) migreres IKKE som innhold —
...
1. **Statushistorikk for skoler:** når en skole bytter status (Aktiv → Pause
   → Tidligere osv.), lagres endringen med dato — i dag overskrives den.
   Kontrollert i koden: avviste påmeldinger settes til `status: 'Inaktiv'`
   (`api/admin/avvis-paamelding.js:65`) og raden overskrives, så det finnes
   ingen historikk over hvem som ble avvist når. ÅPEN (kapittel 6, nr. 7):
   «Inaktiv» står ikke i statuslista (Påmeldt · Aktiv · Aktiv sagt opp ·
   Pause · Tidligere · Potensielle) — enten er lista utdatert eller koden
   bruker et navn som ikke er avtalt. Avklares når historikken bygges.
...
- **Ny databehandleravtale som lister korrekte underleverandører** — i dag:
  Vercel, Supabase, Resend, Bunny.net. (ÅPEN, kapittel 6 nr. 9:
  DPA-avkrysningen.)
...
Hvem tar teknisk ansvar? README-fil, veiledning for ansatte, veiledning for teknisk videreutvikling. (ÅPEN, kapittel 6 nr. 8: hvem tar teknisk ansvar?)
...
kommuner). ÅPEN (kapittel 6, nr. 10): avvikene i Lag B håndteres når
...
1. **Kursholderkalender (mulig — behov ikke avklart, kapittel 6 nr. 11).**
...
ÅPEN (kapittel 6, nr. 12): åpen eller lukket påmelding — og skal modulen
...
ÅPEN (kapittel 6, nr. 14): abonnementsmodell per skole / lærer /
kombinasjon. ÅPEN (nr. 17): Danmark-rapporten er ferdig, men ikke analysert.
...
## Vedlegg A — Endringslogg v33 → v34
